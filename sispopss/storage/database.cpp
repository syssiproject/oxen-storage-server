        ...
        if (db.tableExists("Data")) {
            ...
            while (old_owners.executeStep()) {
                ...
                if (old_owner.size() == 66 && util::starts_with(old_owner, "05") &&
                    sispopc::is_hex(old_owner)) {
                    type = 5;
                    sispopc::from_hex(old_owner.begin() + 2, old_owner.end(), pubkey.begin());
                } else if (old_owner.size() == 64 && sispopc::is_hex(old_owner)) {
                    type = 0;
                    sispopc::from_hex(old_owner.begin(), old_owner.end(), pubkey.begin());
                } else {
                    log::warning(
