...
#include <sispopss/server/utils.h>
...
#include <sispopss/utils/time.hpp>
#include <sispopss/version.h>
#include <sispopss/common/mainnet.h>
#include <sispopss/common/format.h>
...
namespace {
        ...
        for (const auto& sn : swarm->snodes) {
            snodes_json.push_back(
                    json{{"address",  // Deprecated, use pubkey_legacy instead
                          sispopc::to_base32z(sn.pubkey_legacy.view()) + ".snode"},
    ...
    std::string obfuscate_pubkey(const user_pubkey_t& pk) {
        ...
        if (pk_raw.empty())
            return "(none)";
        return sispopc::to_hex(pk_raw.begin(), pk_raw.begin() + 2) + u8"…" +
               sispopc::to_hex(std::prev(pk_raw.end()), pk_raw.end());
    }
    ...
    void register_client_rpc_endpoint(RequestHandler::rpc_map& regs) {
        ...
        calls.load_req = [](std::variant<json, sispopc::bt_dict_consumer> params) -> client_request {
            return std::visit(
        ...
        calls.omq = [](rpc::RequestHandler& h,
            ...
            if (params.empty())
                params = "{}"sv;
            if (params.front() == 'd') {
                req.load_from(sispopc::bt_dict_consumer{params});
                req.b64 = false;
            } else {
    ...
    bool verify_signature(
        ...
        if (subkey) {
            ...
            // Compute our verification key: (c + H("SispopSSSubkey" || c || A)) A and use that
...
std::string compute_hash_blake2b_b64(std::vector<std::string_view> parts) {
    ...
    std::string b64hash = sispopc::to_base64(hash.begin(), hash.end());
    // Trim padding:
    ...
    for (auto& peer : peers) {
        sn.omq_server()->request(
                ...
                [res, peer, cmd](bool success, auto parts) {
                    ...
                    if (good_result) {
                        try {
                            peer_result = server::bt_to_json(sispopc::bt_dict_consumer{parts[0]});
                    ...
                    if (!good_result) {
                        ...
                        if (auto it = peer_result.find("signature");
                            ...
                            *it = sispopc::to_base64(it->get_ref<const std::string&>());
                    }
                ...
                sispopmq::send_option::request_timeout{5s});
    }
}
...
void RequestHandler::process_client_req(rpc::store&& req, std::function<void(Response)> cb) {
#ifndef NDEBUG
    log::trace(logcat, "Storing message: {}", sispopc::to_base64(req.data));
#endif
    ...
    if (success) {
        ...
        mine["signature"] =
                req.b64 ? sispopc::to_base64(sig.begin(), sig.end()) : util::view_guts(sig);
        if (!new_msg)
            mine["already"] = true;
        if (entry_router)
...
void RequestHandler::process_client_req(
        rpc::sispopd_request&& req, std::function<void(rpc::Response)> cb) {
    std::optional<std::string> sispopd_params;
    if (req.params)
        sispopd_params = req.params->dump();

    service_node_.omq_server().sispopd_request(
            "rpc." + req.endpoint,
            [cb = std::move(cb), this](bool success, auto&& data) {
                ...
                if (success && data.size() >= 2 && data[0] == "200") {
                    ...
                    if (result.is_discarded()) {
                        log::warning(
                                logcat,
                                "Invalid sispopd response to client request: result is not valid "
                                "json");
                        return cb({http::BAD_GATEWAY, "sispopd returned unparseable data"s});
                    }
...
void RequestHandler::process_client_req(
    ...
    for (const auto& msg : msgs) {
        messages.push_back(json{
                ...
                {"data", req.b64 ? sispopc::to_base64(msg.data) : std::move(msg.data)},
        });
    }
...
namespace {
    ...
    void handle_action_all_ns(
        ...
        mine["signature"] = b64 ? sispopc::to_base64(sig.begin(), sig.end()) : util::view_guts(sig);
...
void RequestHandler::process_client_req(rpc::delete_msgs&& req, std::function<void(Response)> cb) {
    ...
    mine["signature"] = req.b64 ? sispopc::to_base64(sig.begin(), sig.end()) : util::view_guts(sig);
    if (req.recurse)
...
void RequestHandler::process_storage_test_req(
    ...
    if (status == snode::MessageTestStatus::RETRY) {
        ...
        auto timer = std::make_shared<sispopmq::TimerID>();
...
Response RequestHandler::wrap_proxy_response(
    ...
    if (base64)
        ciphertext = sispopc::to_base64(std::move(ciphertext));
...
void RequestHandler::process_onion_req(RelayToServerInfo&& info, OnionRequestMetadata&& data) {
    ...
    pending_proxy_requests_.emplace_front(cpr::PostCallback(
            ...
            cpr::Header{
                    {"User-Agent",
                     "Sispop Storage Server/" + std::string{STORAGE_SERVER_VERSION_STRING}},
