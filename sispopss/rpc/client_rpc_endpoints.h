...
#include <sispopss/common/pubkey.h>
#include <sispopss/common/namespace.h>
#include <sispopss/common/type_list.h>
...
struct endpoint {
    ...
    virtual void load_from(sispopc::bt_dict_consumer params) = 0;
...
struct no_args : endpoint {
    ...
    void load_from(sispopc::bt_dict_consumer) override {}
};
...
struct recursive : endpoint {
    ...
    virtual sispopc::bt_value to_bt() const = 0;
};
...
///   string `SispopSSSubkey`. The client must therefore sign using `d=a(c+H(c‖A))`, where this `d`
...
struct store final : recursive {
    ...
    void load_from(sispopc::bt_dict_consumer params) override;
    sispopc::bt_value to_bt() const override;
};
...
///
/// Authentication parameters: these are optional during a transition period, up until Sispop
...
/// Forwards an RPC request to the this storage server's sispopd.  Takes keys of:
///
/// - `endpoint` (required) the public sispopd endpoint name such as "ons_resolve". Only accepts
///   whitelisted sispopd rpc endpoints; currently supported are:
///     - get_service_nodes
///     - ons_resolve
/// - `params` (optional) dict of parameters to forward to sispopd.  Can be omitted or null if no
///   parameters should be passed.
///
/// See sispopd rpc documentation (or the sispop-core/src/rpc/core_rpc_server_command_defs.h file) for
/// information on using these sispopd rpc endpoints.
struct sispopd_request final : endpoint {
    static constexpr auto names() { return NAMES("sispopd_request"); }
