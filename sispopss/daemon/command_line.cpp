...
parse_result parse_cli_args(int argc, char* argv[]) {
    ...
    CLI::App cli{"Sispop Storage Server"};
    ...
    if (!(base_dir == fs::path("/var/lib/sispop") || base_dir == fs::path("/var/lib/loki")))
        base_dir /= ".sispop";
    if (testnet) {
        base_dir /= "testnet";
    ...
    options.sispopd_omq_rpc = "ipc://" + (base_dir / "sispopd.sock").u8string();
    ...
    cli.add_option(
               "--sispopd-rpc",
               options.sispopd_omq_rpc,
               "OMQ RPC address on which sispopd is available; typically "
               "ipc:///path/to/sispopd.sock or tcp://localhost:22843")
            ->type_name("OMQ_URL")
            ->capture_default_str();
    cli.add_option(
               "--omq-port,--lmq-port",
               options.omq_port,
               "Public port to listen on for SispopMQ connections")
            ->capture_default_str()
            ->type_name("PORT");
    cli.add_option(
    ...
    cli.add_option(
            ...
            ->type_name("PUBKEY");
    cli.set_version_flag("--version,-v", std::string{sispop::STORAGE_SERVER_VERSION_INFO});
