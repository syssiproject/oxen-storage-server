    ...
    if (version == SERIALIZATION_VERSION_BT) {
        ...
        while (auto* msg = next_msg()) {
            ...
            if (!l.empty() && counter > SERIALIZATION_BATCH_SIZE) {
                ...
                oss << SERIALIZATION_VERSION_BT << sispopc::bt_serializer(l);
                res.push_back(oss.str());
                l.clear();
                counter = 1 + 2 + ser_size;
            }
            assert(msg->pubkey);
            l.push_back(sispopc::bt_list{
        ...
        std::ostringstream oss;
        oss << uint8_t{1} /* version*/ << sispopc::bt_serializer(l);
        res.push_back(oss.str());
    } else {
...
std::vector<message> deserialize_messages(std::string_view slice) {
    ...
    try {
        sispopc::bt_list_consumer l{slice};
        while (!l.is_finished()) {
