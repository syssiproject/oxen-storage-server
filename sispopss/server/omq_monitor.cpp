...
namespace {
    ...
    void monitor_error(sispopc::bt_dict_producer& out, MonitorResponse r, std::string message) {
    ...
    void handle_monitor_message_single(
            sispopc::bt_dict_consumer d, sispopc::bt_dict_producer& out, std::vector<sub_info>& subs) {
        ...
        auto pubkey_hex = sispopc::to_hex(pubkey);
...
void OMQ::handle_monitor_messages(sispopmq::Message& message) {
    if (message.data.size() != 1 || message.data[0].size() < 2 ||
        !(message.data[0].front() == 'd' || message.data[0].front() == 'l') ||
        message.data[0].back() != 'e') {
        message.send_reply(sispopc::bt_serialize(sispopc::bt_dict{
    ...
    try {
        if (m.front() == 'd') {
            result.resize(256);
            sispopc::bt_dict_producer out{result.data(), result.size()};
            handle_monitor_message_single(sispopc::bt_dict_consumer{m}, out, subs);
            result.resize(out.view().size());
        } else {
            result += 'l';
            sispopc::bt_list_consumer l{m};
            std::array<char, 256> buf;
            while (!l.is_finished()) {
                sispopc::bt_dict_producer out{buf.data(), buf.size()};
...
static void write_metadata(
        sispopc::bt_dict_producer& d, std::string_view pubkey, const message& msg) {
    d.append("@", pubkey);
    d.append("h", msg.hash);
...
void OMQ::send_notifies(message msg) {
    ...
    std::vector<sispopmq::ConnectionID> relay_to, relay_to_with_data;
    {
    ...
    if (!relay_to.empty()) {
        sispopc::bt_dict_producer d{data.data(), data.size()};
