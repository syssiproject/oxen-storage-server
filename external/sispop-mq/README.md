# SispopMQ - high-level zeromq-based message passing for network-based projects

This C++17 library contains an abstraction layer around ZeroMQ to provide a high-level interface to
authentication, RPC, and message passing.  It is used extensively within Sispop projects (hence the
name) as the underlying communication mechanism of SN-to-SN communication ("quorumnet"), the RPC
interface used by wallets and local daemon commands, communication channels between sispopd and
...
requires knowing the server pubkey.  Within Sispop, all SN-to-SN traffic is encrypted, and other
...
SispopMQ messages come in two fundamental forms: "commands", consisting of a command named and
...
either direction at any time.  SispopMQ sets up a "listener" and "client" connections, but these only
...
`category.command` - for commands/requests registered by the SispopMQ caller (e.g. sispopd).  Here
...
on this below).  For example, for sispopd categories are:
...
the request via SispopMQ you provide a callback to be invoked when the reply arrives.  On the wire
this looks like:
...
by SispopMQ (both the invoker and the recipient code only see the `hello world`/`dlrow olleh` message
parts).
...
Optional command/request arguments are always strings on the wire.  The SispopMQ-using developer is
...
Internal commands passed between SispopMQ-managed threads use either plain strings or bt-encoded
dictionaries.  See `sispopmq/bt_serialize.h` if you want a bt serializer/deserializer.
...
- a string containing a service node pubkey.  In this mode SispopMQ will look for the given SN in
  already-established connections, reusing a connection if one exists.  If no connection already
  exists, a new connection to the given SN is attempted (this requires constructing the SispopMQ
...
The callbacks are passed a SispopMQ::Message object from which the message (plus various connection
...
connection to deliver the message.  In order for this to work the SispopMQ caller must provide a
lookup function to retrieve the remote address given a SN x25519 pubkey.

### Callbacks

Invoked command functions are always invoked with exactly one arguments: a non-const SispopMQ::Message
reference from which the connection info, SispopMQ object, and message data can be obtained.
...
also allows configuration of how public connections should be treated: for example, an sispopd running
...
For example, in sispopd the categories described above have authentication levels of:
...
In order to handle ServiceNode authentication, SispopMQ uses an Allow callback invoked during
...
SispopMQ operates a pool of worker threads to handle jobs.  The simplest use just allocates new jobs
...
As mentioned above, SispopMQ tries to avoid exceeding the configured general threads value (G)
...
Instead SispopMQ provides a mechanism for this by allowing you to submit a batch of jobs with a
...
exhausting the worker threads with waiting jobs) then you can implement it yourself; SispopMQ isn't
...
SispopMQ supports scheduling periodic tasks via the `add_timer()` function.  These timers have an
