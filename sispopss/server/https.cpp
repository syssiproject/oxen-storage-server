...
#include <sispopss/utils/file.hpp>
#include "utils.h"
#include "omq.h"
...
namespace {
    ...
    struct call_data {
        ...
        call_data(HTTPS& https, sispopmq::SispopMQ& omq, HttpResponse& res) :
    ...
    std::string get_remote_address(HttpResponse& res) {
        ...
        if (addr.size() == 4) {  // IPv4, packed into bytes
            ...
            result << "{unknown:" << sispopc::to_hex(addr) << "}";
        return result.str();
    }
...
void HTTPS::create_endpoints(uWS::SSLApp& https) {
    https.post("/ping_test/v1", [this](HttpResponse* res, HttpRequest* /*req*/) {
        ...
        resp.headers.emplace_back(
                http::SNODE_PUBKEY_HEADER, sispopc::to_base64(legacy_keys_.first.view()));
...
bool HTTPS::should_rate_limit_client(std::string_view addr) {
    ...
    sispopc::big_to_host_inplace(ip);
