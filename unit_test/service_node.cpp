...
static sispop::snode::sn_record create_dummy_sn_record() {
...
static void test_ip_update(ip_ports old_addr, ip_ports new_addr, ip_ports expected) {
    using sispop::snode::sn_record;

    auto sn = create_dummy_sn_record();

    std::tie(sn.ip, sn.port, sn.omq_port) = old_addr;

    sispop::snode::SwarmInfo si{0, std::vector<sn_record>{sn}};
    std::vector<sispop::snode::SwarmInfo> current{{si}};

    std::tie(sn.ip, sn.port, sn.omq_port) = new_addr;

    sispop::snode::SwarmInfo si2{0, std::vector<sn_record>{sn}};
    std::vector<sispop::snode::SwarmInfo> incoming{{si2}};
...
TEST_CASE("service nodes - message hashing", "[service-nodes][messages]") {
    ...
    const auto data = sispopc::from_base64(
    ...
    CHECK(sispop::rpc::computeMessageHash(pk, sispop::namespace_id::Default, data) == expected);
    CHECK(sispop::rpc::compute_hash_blake2b_b64({pk.prefixed_raw() + data}) == expected);
}
