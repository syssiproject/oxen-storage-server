...
class RequestHandler {
  ...
  public:
    ...
    void process_client_req(rpc::sispopd_request&& req, std::function<void(Response)> cb);
    ...
    struct rpc_handler {
        std::function<client_request(std::variant<nlohmann::json, sispopc::bt_dict_consumer> params)>
                load_req;
    ...
    // Forwards a request to sispopd RPC. `params` should contain:
    // - endpoint -- the name of the rpc endpoint; currently allowed are `ons_resolve` and
    // `get_service_nodes`.
    // - params -- optional dict of parameters to pass through to sispopd as part of the request
    //
    // See sispop-core/rpc/core_rpc_server_command_defs.h for parameters to these endpoints.
    //
    // Returns (via the response callback) the sispopd JSON object on success; on failure returns
    // a failure response with a body of the error string.
    void process_sispopd_request(const nlohmann::json& params, std::function<void(Response)> cb);
