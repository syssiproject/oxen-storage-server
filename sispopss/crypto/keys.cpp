...
namespace detail {
    void load_from_hex(void* buffer, size_t length, std::string_view hex) {
        ...
        sispopc::from_hex(hex.begin(), hex.end(), reinterpret_cast<unsigned char*>(buffer));
    }
    ...
    std::string to_hex(const unsigned char* buffer, size_t length) {
        return sispopc::to_hex(buffer, buffer + length);
    }

}  // namespace detail

std::string ed25519_pubkey::snode_address() const {
    auto addr = sispopc::to_base32z(begin(), end());
    addr += ".snode";
    return addr;
}
...
static T parse_pubkey(std::string_view pubkey_in) {
    ...
    else if (pubkey_in.size() == 64 && sispopc::is_hex(pubkey_in))
        sispopc::from_hex(pubkey_in.begin(), pubkey_in.end(), pk.begin());
    else if (
            (pubkey_in.size() == 43 || (pubkey_in.size() == 44 && pubkey_in.back() == '=')) &&
            sispopc::is_base64(pubkey_in))
        sispopc::from_base64(pubkey_in.begin(), pubkey_in.end(), pk.begin());
    else if (pubkey_in.size() == 52 && sispopc::is_base32z(pubkey_in))
        sispopc::from_base32z(pubkey_in.begin(), pubkey_in.end(), pk.begin());
    else {
...
std::array<unsigned char, 32> subkey_verify_key(
    ...
    // Need to compute: (c + H("SispopSSSubkey" || c || A)) A and use that instead of A for
    // verification:

    // H("SispopSSSubkey" || c || A):
