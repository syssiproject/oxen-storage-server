...
namespace {
    ...
    void load_pk_signature(
        ...
        if (pk_ed) {
            ...
            if (pk_ed->size() == 64) {
                if (!sispopc::is_hex(*pk_ed))
                    throw parse_error{"invalid pubkey_ed25519: value is not hex"};
                sispopc::from_hex(pk_ed->begin(), pk_ed->end(), rpc.pubkey_ed25519.emplace().begin());
    ...
    void load_subkey(RPC& rpc, const Dict&, const std::optional<std::string_view>& subkey) {
        ...
        if constexpr (std::is_same_v<json, Dict>) {
            if (sispopc::is_base64(sk) && (sk.size() == 43 || (sk.size() == 44 && sk.back() == '=')))
                sispopc::from_base64(sk.begin(), sk.end(), rpc.subkey.emplace().begin());
            else if (sispopc::is_hex(sk) && sk.size() == 64)
                sispopc::from_hex(sk.begin(), sk.end(), rpc.subkey.emplace().begin());
            else
                throw parse_error{
...
static void load(store& s, Dict& d) {
    ...
    if constexpr (std::is_same_v<Dict, json>) {
        ...
        s.data = sispopc::from_base64(*data);
    } else {
...
static void load(retrieve& r, Dict& d) {
    ...
    if (last_hash) {
        if (last_hash->empty())  // Treat empty string as not provided
            ...
            if (!sispopc::is_base64(*last_hash))
...
static bool is_valid_message_hash(std::string_view hash) {
    return (hash.size() == 43 && sispopc::is_base64(hash));
}
...
static void load(revoke_subkey& rs, Dict& d) {
    ...
    if constexpr (std::is_same_v<json, Dict>) {
        if (sispopc::is_base64(sk) && (sk.size() == 43 || (sk.size() == 44 && sk.back() == '=')))
            sispopc::from_base64(sk.begin(), sk.end(), rs.revoke_subkey.begin());
        else if (sispopc::is_hex(sk) && sk.size() == 64)
            sispopc::from_hex(sk.begin(), sk.end(), rs.revoke_subkey.begin());
        else
...
inline const static std::unordered_set<std::string_view> allowed_sispopd_endpoints{
        {"get_service_nodes"sv, "ons_resolve"sv}};

template <typename Dict>
static void load(sispopd_request& o, Dict& d) {
    auto endpoint = parse_field<std::string>(d, "endpoint");
    require("endpoint", endpoint);
    o.endpoint = *endpoint;
    if (!allowed_sispopd_endpoints.count(o.endpoint))
        throw parse_error{fmt::format("Invalid sispopd endpoint '{}'", o.endpoint)};
    ...
    if constexpr (std::is_same_v<Dict, json>) {
        ...
        if (auto json_str = parse_field<std::string_view>(d, "params")) {
            ...
            if (params.is_discarded())
                throw parse_error{"sispopd_request params field does not contain valid json"};
            if (!params.is_null())
                o.params = std::move(params);
        }
    }
}
void sispopd_request::load_from(json params) {
    load(*this, params);
}
void sispopd_request::load_from(bt_dict_consumer params) {
    load(*this, params);
}
