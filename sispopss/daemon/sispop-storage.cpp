...
#include <sispopss/rpc/sispopd_rpc.h>
#include <sispopss/rpc/request_handler.h>
#include <sispopss/server/https.h>
#include <sispopss/server/omq.h>
#include <sispopss/server/server_certificates.h>
#include <sispopss/snode/service_node.h>
#include <sispopss/snode/swarm.h>
...
static auto logcat = sispop::log::Cat("daemon");
...
int main(int argc, char* argv[]) {
    ...
    log::info(logcat, "Connecting to sispopd @ {}", options.sispopd_omq_rpc);
    ...
    try {
        ...
        const auto [private_key, private_key_ed25519, private_key_x25519] =
                rpc::get_sn_privkeys(options.sispopd_omq_rpc, [] { return signalled == 0; });
        ...
        log::info(logcat, "Retrieved keys from sispopd; our SN pubkeys are:");
        ...
        // Set up sispopmq now, but don't actually start it until after we set up the ServiceNode
        // instance (because ServiceNode and SispopmqServer reference each other).
        auto sispopmq_server_ptr =
                std::make_unique<server::OMQ>(me, private_key_x25519, stats_access_keys);
        auto& sispopmq_server = *sispopmq_server_ptr;

        snode::ServiceNode service_node{
                me, private_key, sispopmq_server, options.data_dir, options.force_start};

        rpc::RequestHandler request_handler{service_node, channel_encryption, private_key_ed25519};

        rpc::RateLimiter rate_limiter{*sispopmq_server};
        ...
        sispopmq_server.init(
                &service_node,
                &request_handler,
                &rate_limiter,
                sispopmq::address{options.sispopd_omq_rpc});

        https_server.start();

#ifdef ENABLE_SYSTEMD
        sd_notify(0, "READY=1");
        sispopmq_server->add_timer(
                [&service_node] {
