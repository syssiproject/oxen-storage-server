...
#include <sispopss/version.h>
#include <sispopss/common/mainnet.h>
#include <sispopss/rpc/request_handler.h>
#include <sispopss/server/omq.h>
#include <sispopss/logging/sispop_logger.h>
#include <sispopss/utils/string_utils.hpp>
#include <sispopss/utils/random.hpp>
...
constexpr std::chrono::seconds SISPOPD_PING_INTERVAL = 30s;
...
ServiceNode::ServiceNode(
    ...
    auto delay_timer = std::make_shared<sispopmq::TimerID>();
...
void ServiceNode::on_sispopd_connected() {
    auto started = std::chrono::steady_clock::now();
    update_swarms();
    sispopd_ping();
    omq_server_->add_timer([this] { sispopd_ping(); }, SISPOPD_PING_INTERVAL);
    ...
    while (true) {
        if (first_response_cv_.wait_for(lock, 5s, [this] { return got_first_response_; })) {
            log::info(
                    logcat,
                    "Got initial block update from sispopd in {}",
                    util::short_duration(std::chrono::steady_clock::now() - started));
            break;
        }
        log::warning(logcat, "Still waiting for initial block update from sispopd...");
    }
}
...
static block_update parse_swarm_update(const std::string& response_body) {
    if (response_body.empty()) {
        log::critical(logcat, "Bad sispopd rpc response: no response body");
    ...
    try {
        ...
        for (const auto& sn_json : service_node_states) {
            ...
            if (pk_x25519_hex.empty() || pk_ed25519_hex.empty()) {
                ...
                // them yet (or our local sispopd hasn't received them yet).
                missing_aux_pks++;
                log::debug(
                        logcat,
        ...
        if (missing_aux_pks >
            ...
            log::warning(
                    ...
                    "sispopd may be out of sync with the network",
                    missing_aux_pks,
                    total);
        }
    } catch (const std::exception& e) {
        log::critical(logcat, "Bad sispopd rpc response: invalid json ({})", e.what());
        throw std::runtime_error("Failed to parse swarm update");
    }
...
void ServiceNode::bootstrap_data() {
    ...
    std::vector<sispopmq::address> seed_nodes;
    if (sispop::is_mainnet) {
        seed_nodes.emplace_back(
                "curve://public.sispop.site:22027/"
    ...
    for (const auto& addr : seed_nodes) {
        auto connid = omq_server_->connect_remote(
                addr,
                [addr](sispopmq::ConnectionID) {
                    log::debug(logcat, "Connected to bootstrap node {}", addr.full_address());
                },
                [addr](sispopmq::ConnectionID, auto reason) {
                    log::debug(
                            logcat,
                ...
                sispopmq::connect_option::ephemeral_routing_id{true},
                sispopmq::connect_option::timeout{BOOTSTRAP_TIMEOUT});
        omq_server_->request(
                connid,
                ...
                params,
                sispopmq::send_option::request_timeout{BOOTSTRAP_TIMEOUT});
    }
}
...
void ServiceNode::send_onion_to_sn(
    ...
    omq_server_->request(
            ...
            "sn.onion_request",
            std::move(cb),
            sispopmq::send_option::request_timeout{30s},
...
void ServiceNode::update_swarms() {
    ...
    omq_server_.sispopd_request(
            "rpc.get_service_nodes",
            [this](bool success, std::vector<std::string> data) {
                updating_swarms_ = false;
                if (!success || data.size() < 2) {
                    log::critical(logcat, "Failed to contact local sispopd for service node list");
                    return;
                }
                try {
                    std::lock_guard lock{sn_mutex_};
                    ...
                    if (!got_first_response_) {
                        log::info(logcat, "Got initial swarm information from local Sispopd");
                        ...
                        if (total >= (sispop::is_mainnet ? 100 : 10) &&
                            missing <= MISSING_PUBKEY_THRESHOLD::num * total /
                                               MISSING_PUBKEY_THRESHOLD::den) {
                            log::info(
                                    logcat,
                                    "Initialized from sispopd with {}/{} SN records",
                                    total - missing,
                                    total);
                            syncing_ = false;
                        } else {
...
void ServiceNode::test_reachability(const sn_record& sn, int previous_failures) {
    ...
    if (sn.ip == "0.0.0.0") {
        // sispopd won't accept 0.0.0.0 in an uptime proof, which means if we see this the node
        // hasn't sent an uptime proof; we could treat it as a failure, but that seems
        // unnecessary since sispopd will already fail the service node for not sending uptime
        // proofs.
    ...
    // (in which case they have to report the final result to sispopd).
    ...
    cpr::Header headers{
            {"Host",
             sn.pubkey_ed25519 ? sispopc::to_base32z(sn.pubkey_ed25519.view()) + ".snode"
                               : "service-node.snode"},
            {"Content-Type", "application/octet-stream"},
            {"User-Agent", "Sispop Storage Server/" + std::string{STORAGE_SERVER_VERSION_STRING}},
    };
    ...
    omq_server_->request(
            ...
            [this, test_results = std::move(test_results), previous_failures](
                ...
                log::debug(
                        logcat,
                        "{} response for SispopMQ ping test of {}",
            ...
            sispopmq::send_option::request_timeout{SN_PING_TIMEOUT});
}

void ServiceNode::sispopd_ping() {
    std::lock_guard guard(sn_mutex_);

    json sispopd_params{
            {"version", STORAGE_SERVER_VERSION},
            {"pubkey_ed25519",
             sispopc::to_hex(our_address_.pubkey_ed25519.begin(), our_address_.pubkey_ed25519.end())},
    ...
    omq_server_.sispopd_request(
            ...
            [this](bool success, std::vector<std::string> data) {
                if (!success)
                    log::critical(
                            logcat, "Could not ping sispopd: Request failed ({})", data.front());
                else if (data.size() < 2 || data[1].empty())
                    log::critical(logcat, "Could not ping sispopd: Empty body on reply");
                else
                    try {
                        if (const auto status =
                                    json::parse(data[1]).at("status").get<std::string>();
                            status == "OK") {
                            auto good_pings = ++sispopd_pings_;
                            if (good_pings == 1)  // First ping after startup or after ping failure
                                log::info(logcat, "Successfully pinged sispopd");
                            else if (good_pings % (1h / SISPOPD_PING_INTERVAL) == 0)  // Once an hour
                                log::info(logcat, "{} successful sispopd pings", good_pings);
                            else
                                log::debug(
                                        logcat,
                                        "Successfully pinged Sispopd ({} consecutive times)",
                                        good_pings);
                        } else {
                            log::critical(logcat, "Could not ping sispopd: {}", status);
                            sispopd_pings_ = 0;
                        }
                    } catch (...) {
                        log::critical(logcat, "Could not ping sispopd: bad json in response");
                    }
            },
            sispopd_params.dump());

    // Also re-subscribe (or subscribe, in case sispopd restarted) to block subscriptions.  This
    // makes sispopd start firing notify.block messages at as whenever new blocks arrive, but we
    ...
    omq_server_.sispopd_request("sub.block", [](bool success, auto&& result) {
        if (!success || result.empty())
            log::critical(
                    logcat,
                    "Failed to subscribe to sispopd block notifications: {}",
                    result.empty() ? "response is empty" : result.front());
        else if (result.front() == "OK")
            log::info(logcat, "Subscribed to sispopd new block notifications");
        else if (result.front() == "ALREADY")
            log::debug(logcat, "Renewed sispopd new block notification subscription");
    });
}
...
void ServiceNode::send_storage_test_req(
    ...
    bool is_b64 = sispopc::is_base64(msg.hash);
    if (!is_b64) {
        log::error(
                logcat,
    ...
    omq_server_->request(
            ...
            sispopmq::send_option::request_timeout{STORAGE_TEST_TIMEOUT},
            // Data parts: test height and msg hash (in bytes)
            std::to_string(block_height_),
            sispopc::from_base64(msg.hash));
}
...
void ServiceNode::report_reachability(const sn_record& sn, bool reachable, int previous_failures) {
    auto cb = [sn_pk = sn.pubkey_legacy, reachable](bool success, std::vector<std::string> data) {
        ...
        if (data.size() < 2 || data[1].empty()) {
            log::warning(logcat, "Empty body on Sispopd report node status");
            return;
        }
    ...
    omq_server_.sispopd_request("admin.report_peer_status", std::move(cb), params.dump());
...
std::optional<std::pair<sn_record, sn_record>> ServiceNode::derive_tester_testee(
    ...
    std::mt19937_64 mt{sispopc::load_little_to_host<uint64_t>(block_hash.data())};
