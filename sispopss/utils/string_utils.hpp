...
// Copied entirely from sispop-core, minus the epee bits.
...
/// want to do unless the struct is specifically design to be used this way.  The value must be
/// a standard layout type; it should really require is_trivial, too, but we have classes (like
/// crypto keys) that aren't C++-trivial but are still designed to be accessed this way.
template <typename T>
